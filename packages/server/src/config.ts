...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.MAYA_SERVER_CONFIG;
  ...
  const envWorkspaces = parseList(process.env.MAYA_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.MAYA_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.MAYA_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.MAYA_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.MAYA_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.MAYA_TOKEN;
  const hostTokenFromEnv = process.env.MAYA_HOST_TOKEN;
  ...
  const approvalMode =
    cli.approvalMode ??
    (process.env.MAYA_APPROVAL_MODE as ApprovalMode | undefined) ??
    fileConfig.approval?.mode ??
    "manual";

  const approvalTimeoutMs =
    cli.approvalTimeoutMs ??
    (process.env.MAYA_APPROVAL_TIMEOUT_MS ? Number(process.env.MAYA_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.MAYA_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.MAYA_READONLY;
  ...
  const envLogFormat = process.env.MAYA_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.MAYA_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.MAYA_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.MAYA_PORT ? Number(process.env.MAYA_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
