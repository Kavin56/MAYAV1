...
const DEFAULT_MAYA_PORT: u16 = 8787;

pub fn resolve_maya_port() -> Result<u16, String> {
    if TcpListener::bind(("0.0.0.0", DEFAULT_MAYA_PORT)).is_ok() {
        return Ok(DEFAULT_MAYA_PORT);
    }
...
pub fn spawn_maya_server(
    ...
    if let Some(username) = opencode_username {
        if !username.trim().is_empty() {
            command = command.env("MAYA_OPENCODE_USERNAME", username);
        }
    }

    if let Some(password) = opencode_password {
        if !password.trim().is_empty() {
            command = command.env("MAYA_OPENCODE_PASSWORD", password);
        }
    }
