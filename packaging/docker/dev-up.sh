...
MAYA_PORT="$(pick_port)"
WEB_PORT="$(pick_port)"
if [ "$WEB_PORT" = "$MAYA_PORT" ]; then
  WEB_PORT="$(pick_port)"
fi

echo "Starting Docker Compose project: $PROJECT" >&2
echo "- MAYA_PORT=$MAYA_PORT" >&2
echo "- WEB_PORT=$WEB_PORT" >&2

MAYA_DEV_ID="$DEV_ID" MAYA_PORT="$MAYA_PORT" WEB_PORT="$WEB_PORT" \
  docker compose -p "$PROJECT" -f "$COMPOSE_FILE" up -d

echo "" >&2
echo "MAYA web UI:     http://localhost:$WEB_PORT" >&2
echo "MAYA server:     http://localhost:$MAYA_PORT" >&2
